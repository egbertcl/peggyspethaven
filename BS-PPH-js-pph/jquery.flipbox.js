(function($) {
  ...
  function FlipBox($element, options) {
    ...
    this.contentIndex =
      Math.min(Math.max(options.index, 0), this.contents.length - 1) || 0;
  ...
  FlipBox.prototype.create = function() {
    ...
    this.$front = $('<div></div>')
      .addClass('flipbox-side flipbox-front')
      .appendTo(this.$box);
    this.$back = $('<div></div>')
      .addClass('flipbox-side flipbox-back')
      .appendTo(this.$box);

    if (this.options.vertical) {
      this.$box.addClass('flipbox-vertical');
      this.$top = $('<div></div>')
        .addClass('flipbox-side flipbox-top')
        .appendTo(this.$box);
      this.$bottom = $('<div></div>')
        .addClass('flipbox-side flipbox-bottom')
        .appendTo(this.$box);
    } else {
      this.$box.addClass('flipbox-horizontal');
      this.$left = $('<div></div>')
        .addClass('flipbox-side flipbox-left')
        .appendTo(this.$box);
      this.$right = $('<div></div>')
        .addClass('flipbox-side flipbox-right')
        .appendTo(this.$box);
    }
  ...
  FlipBox.prototype.resize = function() {
    if (this.options.vertical) {
      this.$box.css(
        'transform-origin',
        '0 ' + this.options.height / 2 + 'px -' + this.options.height / 2 + 'px'
      );
      this.$back.css(
        'transform',
        'translateZ(-' + this.options.height + 'px) rotateX(180deg)'
      );
      this.$top.css(
        'transform',
        'rotateX(-270deg) translateY(-' + this.options.height + 'px)'
      );
      this.$bottom.css(
        'transform',
        'rotateX(-90deg) translateY(' + this.options.height + 'px)'
      );
    } else {
      this.$box.css(
        'transform-origin',
        this.options.width / 2 + 'px 0 -' + this.options.width / 2 + 'px'
      );
      this.$back.css(
        'transform',
        'translateZ(-' + this.options.width + 'px) rotateY(180deg)'
      );
      this.$left.css(
        'transform',
        'rotateY(270deg) translateX(-' + this.options.width + 'px)'
      );
      this.$right.css(
        'transform',
        'rotateY(-270deg) translateX(' + this.options.width + 'px)'
  ...
  FlipBox.prototype.addContent = function(newContent, contentIndex) {
    contentIndex =
      contentIndex || contentIndex === 0 ? contentIndex : this.contents.length;
  ...
  FlipBox.prototype.replaceContent = function(newContent, contentIndex) {
    contentIndex =
      contentIndex || contentIndex === 0 ? contentIndex : this.contentIndex;
    contentIndex = Math.min(Math.max(0, contentIndex), this.contents.length);
  ...
  FlipBox.prototype.next = function(reverse) {
    this.displayContent(
      this.contentIndex + 1 < this.contents.length ? this.contentIndex + 1 : 0,
      reverse
    );
  };
  ...
  FlipBox.prototype.prev = function(reverse) {
    this.displayContent(
      this.contentIndex > 0 ? this.contentIndex - 1 : this.contents.length - 1,
      reverse
    );
  };
  ...
  FlipBox.prototype.jump = function(index, reverse) {
    this.displayContent(
      Math.min(Math.max(index, 0), this.contents.length - 1),
      reverse
    );
  };
